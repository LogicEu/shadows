...
float GetDist(vec3 p) {
    ...
    wake *= S(0., -1., x);
	float gyroid = (min(dot(sin(p2), cos(p2.zxy)),wake)-.4)/10.;
...
void main(void)
{
    ...
    ro.xz *= Rot(-m.x*6.2831+u_time*.05);
    ro.y = max(ro.y, -.9);
    ...
    for(int x=0; x<AA; x++) {
        for(int y=0; y<AA; y++) {
            ...
            vec2 uv = (gl_FragCoord.xy+offs-.5*u_resolution.xy)/u_resolution.y;
            ...
            float vein = S(-.01,.02, Gyroid(P+sin(P*30.+u_time)*.01)+.03);
            sss *= vein;
            col += sss*vec3(1,.1,.1);
    ...
    float pulse = pow(sin(u_time)*.5+.5, 150.);
    t = u_time;
